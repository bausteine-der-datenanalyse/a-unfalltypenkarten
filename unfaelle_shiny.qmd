...
read_unfaelle <- function(f) {
  read_csv2(f) |>
    ...
    mutate(
      UWOCHENTAG = lubridate::wday(as.numeric(UWOCHENTAG), label=TRUE),
      UMONAT = lubridate::month(as.numeric(UMONAT), label=TRUE , abbr = FALSE)
    )
}
...
Bochum_tbl <- Bochum_Filter() %>% 
  ...
  summarise(
    `PKW-Beteiligung` = sum(IstPKW == "1") , 
    `Rad-Beteiligung` = sum(IstRad == "1") , 
    `Fußgänger-Beteiligung` = sum(IstFuss == "1")) %>%
